...
def dropout_forward(x, dropout_param):
    ...
    if mode == "train":
        ...
        #######################################################################
        mask = 1 / (1 - p) * (np.random.rand(*x.shape) >= p)
        out = x * mask
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
        ...
        for f in range(F):
            for hh in range(H_prime):
                for ww in range(W_prime):
                    ...
                    h_end = h_start + HH
                    w_end = w_start + WW
                    out[n, f, hh, ww] = np.sum(x_pad[n, :, h_start : h_end, w_start : w_end] * w[f, :, :, :]) + b[f]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
        ...
        for f in range(F):
            for hh in range(H_prime):
                for ww in range(W_prime):
                    ...
                    # out[n, f, hh, ww] = np.sum(x_pad[n, :, h_start : h_end, w_start : w_end] * w[f, :, :, :]) + b[f]
                    db[f] += dout[n, f, hh, ww]
                    dw[f, :, :, :] += dout[n, f, hh, ww] * x_pad[n, :, h_start : h_end, w_start : w_end]
                    dx_pad[n, :, h_start : h_end, w_start : w_end] += dout[n, f, hh, ww] * w[f, :, :, :]
        ...
        for c in range(C):
            for hh in range(H_prime):
                for ww in range(W_prime):
                    ...
                    out[n, c, hh, ww] = np.max(x[n, c, h_start : h_end, w_start : w_end])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
        ...
        for c in range(C):
            for hh in range(H_prime):
                for ww in range(W_prime):
                    ...
                    w_start = ww * stride
                    w_end = w_start + pool_width
                    out = np.max(x[n, c, h_start : h_end, w_start : w_end])
                    for hhh in range(h_start, h_end):
                        for www in range(w_start, w_end):
                            if x[n, c, hhh, www] == out:
                                dx[n, c, hhh, www] = dout[n, c, hh, ww]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    N, C, H, W = x.shape
    Cg = C // G
    x = x.reshape(N, Cg, G, H, W).transpose(0, 2, 3, 4, 1).reshape(-1, Cg)
    dummy_gamma = np.ones(Cg)
    dummy_beta = np.zeros(Cg)
    out, ln_cache = layernorm_forward(x, dummy_gamma, dummy_beta, ln_param={"eps": eps})
    x_norm = out.reshape(N, G, H, W, Cg).transpose(0, 4, 1, 2, 3).reshape(N, C, H, W)
    out = x_norm * gamma + beta
    cache = (ln_cache, x_norm, Cg, G, gamma, beta)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def spatial_groupnorm_backward(dout, cache):
    ...
    # This will be extremely similar to the layer norm implementation.        #
    ###########################################################################
    ln_cache, x_norm, Cg, G, gamma, beta = cache
    N, C, H, W = dout.shape
    dgamma = np.sum(dout * x_norm, axis=(0, 2, 3), keepdims=True)
    dbeta = dout.sum(axis=(0, 2, 3), keepdims=True)
    dx_norm = dout * gamma
    dx_norm = dx_norm.reshape(N, Cg, G, H, W).transpose(0, 2, 3, 4, 1).reshape(-1, Cg)
    dx, _, _ = layernorm_backward(dx_norm, ln_cache)
    dx = dx.reshape(N, G, H, W, Cg).transpose(0, 4, 1, 2, 3).reshape(N, C, H, W)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dgamma, dbeta
